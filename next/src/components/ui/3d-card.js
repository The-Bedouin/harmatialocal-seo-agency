...
export function CardBody({ children, className = "", ...props }) {
  ...
  const rotateY = useTransform(x, [0, 1], [-8, 8]);
  return (
    <motion.div className={`transition-transform duration-300 ${className}`} style={{ rotateX, rotateY }} {...props}>
      {children}
    </motion.div>
  );
}
...
export function CardItem({ children, className = "", translateZ = 0, as: Component = "div", ...props }) {
  return (
    <motion.div className={className} style={{ translateZ }} {...props} as={Component}>
      {children}
    </motion.div>
  );
}
